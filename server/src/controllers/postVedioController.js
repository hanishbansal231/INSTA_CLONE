import postVideoModel from '../models/postVideoModel.js';
...
export const createPostReel = asyncHandler(async (req, res, next) => {
    try {
        ...
        const postVedio = await postVideoModel.create({
            tag,
            description,
            user: id,
        });
        ...
        if (req.file) {
            try {
                const vedio = req.file;

                const res = await uplodaImageCloudinary(vedio.path);

                postVedio.reel.public_id = res.public_id;
                postVedio.reel.secure_url = res.secure_url;

                await postVedio.save();

            } catch (error) {
                console.log(error);
                return next(new apiError(402, 'Vedio upload Failed...'));
            }
        }
        ...
        return res.status(201).json(
            new apiResponse(200, postVedio, 'Post Created Successfully')
        )
...
export const getAllReelPost = asyncHandler(async (req, res, next) => {
    try {
        ...
        const allPosts = await postVideoModel.find({}).skip((page - 1) * limit).limit(limit).sort({ createdAt: -1 });
...
export const getSingleReelPost = asyncHandler(async (req, res, next) => {
    try {
        ...
        const vedio = await postVideoModel.findById({ _id: id });
        ...
        return res.status(201).json(
            new apiResponse(200, vedio, 'Post find succcessfully...')
        )
